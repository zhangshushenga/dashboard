<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">婴幼儿哭泣</div>
...
import { generateInfantCry, getInfantCry, getInfantCryImage } from '@/api/agents'
...
export default {
  ...
  methods: {
    ...
    getUpload() {
      // get 上传地址
      getInfantCryImage().then(response => {
    ...
    async uploadToS3(request) {
      ...
      try {
        ...
        const response = await getInfantCryImage()
    ...
    queryAction() {
      ...
      generateInfantCry(param).then(response => {
        if (response.code === 0) {
          // 获取 requestId
    ...
    checkAndDownloadFile: function(requestId, manual = true) {
      ...
      getInfantCry(requestId).then(response => {
