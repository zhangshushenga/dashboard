<template>
    <div class="container">
        <div style="display: flex; flex: 0.7; flex-direction: row;" class="background-image_nav">
            <div class="currentTime">{{ currentDateTime }}</div>
            <div class="title">华来视觉语言大模型 V15NX</div>
            <div class="currentTime"></div>
        </div>
        <div style="display: flex; flex: 9;">
            <cub ref="cubRef" :currentAgentsCall="currentAgentsCall" :totalAgentsCall="totalAgentsCall"
                :agentTotalNum="agentTotalNum" :currentRequestAgentNum="currentRequestAgentNum"
                :smallCubRoatSpeed="smallCubRoatSpeed" />
        </div>
        <div class="GPUList">
        ...
        <div style="width: 100%; height: 100%; position: absolute; left: 0; top: 0" @click="gotoIntelligent"></div>
    </div>
</template>

<script>
import cub from './cub.vue';
...
export default {
    ...
    mounted() {
        ...
        this.interval1 = setInterval(this.getCurrentDateTime, 1000);
    ...
    methods: {
        ...
        agentsData() {
            ...
            this.agentTimer = setTimeout(() => {
                ...
                this.interval2 = setInterval(this.agentRequest, 24 * 60 * 60 * 1000);
            ...
            this.interval3 = setInterval(async () => {
                ...
                this.$refs.cubRef.updateUsingAgent()

            }, frequency1 * 1000);

            this.interval4 = setInterval(async () => {
                const result = await this.agentStatRequest()
            }, frequency3 * 1000);
        ...
        gotoIntelligent() {
            // this.currentAgentsCall = this.currentAgentsCall * 3
            this.incrementValue()
            this.incrementValue1()
            this.$emit('gotoIntelligent');
        },
        incrementValue() {
            const target = this.currentAgentsCall * 3;
            const totalDuration = 7000; // 总时间（毫秒）
            const intervalDuration = 100; // 每次递增的间隔时间（毫秒）
            const steps = totalDuration / intervalDuration; // 计算总步数
            const incrementPerStep = (target - this.currentAgentsCall) / steps; // 每步增量

            const interval = setInterval(() => {
                if (this.currentAgentsCall < target) {
                    this.currentAgentsCall += incrementPerStep;
                } else {
                    this.currentAgentsCall = target; // 确保不超过目标值
                    clearInterval(interval);
                }
            }, intervalDuration);
        },
        incrementValue1() {
            const target = this.smallCubRoatSpeed * 13;
            const totalDuration = 7000; // 总时间（毫秒）
            const intervalDuration = 100; // 每次递增的间隔时间（毫秒）
            const steps = totalDuration / intervalDuration; // 计算总步数
            const incrementPerStep = (target - this.smallCubRoatSpeed) / steps; // 每步增量

            const interval = setInterval(() => {
                if (this.smallCubRoatSpeed < target) {
                    this.smallCubRoatSpeed += incrementPerStep;
                } else {
                    this.smallCubRoatSpeed = target; // 确保不超过目标值
                    clearInterval(interval);
                }
            }, intervalDuration);
        }
