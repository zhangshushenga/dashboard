<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">车辆智识</div>
    ...
    <div style="margin: 30px">
      ...
      <el-descriptions :column="1" title="结果">
        <el-descriptions-item v-if="this.result.evaluation === 1" label="触发"><span style="color:green">是</span>
        </el-descriptions-item>
        <el-descriptions-item v-if="this.result.evaluation === 2" label="触发"><span style="color:red">非</span>
        </el-descriptions-item>

        <el-descriptions-item v-if="this.result.duration > 0 " label="模型耗时"> {{ this.result.duration }} ms
        </el-descriptions-item>
        <el-descriptions-item label="描述" label-style="min-width:50px">{{
...
import { generateCarRecognize, getCarRecognize, getCarRecognizeImage } from '@/api/agents'
...
export default {
  ...
  methods: {
    ...
    getList(currentPage) {
      getCarRecognize(4).then(response => {
    ...
    getUpload() {
      // get 上传地址
      getCarRecognizeImage().then(response => {
    ...
    async uploadToS3(request) {
      ...
      try {
        ...
        const response = await getCarRecognizeImage()
    ...
    queryAction() {
      ...
      generateCarRecognize(param).then(response => {
        if (response.code === 0) {
          // 获取 requestId
    ...
    checkAndDownloadFile: function(requestId, manual = true) {
      ...
      getCarRecognize(requestId).then(response => {
        ...
        if (data.status !== 0) {
          ...
          this.result.evaluation = this.result.result.evaluation
          return
        }
        if (timerElement) {
