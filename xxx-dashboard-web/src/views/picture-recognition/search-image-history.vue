<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">搜索图片历史</div>
    <div style="display: flex; flex-direction: row;">
      <div style="display: flex; flex-direction: row; flex-wrap: wrap; flex: 7">
        <el-input v-model="deviceId" class="queryItem" disabled placeholder="请输入deviceId / 设备id进行搜索" />
        <el-input v-model="content" class="queryItem" disabled placeholder="搜索内容" style="width: 50%" />
      </div>
    </div>
    <div style="display: flex; flex-direction: row; flex-wrap: wrap;">
      <div v-for="data in currentData" :key="data.id" class="imageCard">
        ...
        <div v-if="data.matchResult != undefined" class="content">是否符合搜索结果: {{ getMatchResultText(data.matchResult) }}
        </div>
        <div
        ...
        >
          <div>短描述:</div>
          <el-popover
            v-for="shortLabelInfo in data.shortLabelInfoList"
            :content="shortLabelInfo.roi"
            placement="top-start"
            title="坐标"
            trigger="hover"
          >
