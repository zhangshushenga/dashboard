<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">鸟类识别</div>
...
import { generateBirdsEye, getBirdsEye, getBirdsEyeImage } from '@/api/agents'
...
export default {
  ...
  methods: {
    ...
    getUpload() {
      // get 上传地址
      getBirdsEyeImage().then(response => {
    ...
    async uploadToS3(request) {
      ...
      try {
        ...
        const response = await getBirdsEyeImage()
    ...
    queryAction() {
      ...
      generateBirdsEye(param).then(response => {
        if (response.code === 0) {
          // 获取 requestId
    ...
    checkAndDownloadFile: function(requestId, manual = true) {
      ...
      getBirdsEye(requestId).then(response => {
