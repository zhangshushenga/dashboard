        ...
        <el-container style=" background-color: #ffffff; border-radius:6px ;">
            <el-main>
                <el-table :data="tableData"
                    ...
                    <el-table-column align="left" label="配对数" min-width="10%">
                        <template slot-scope="scope">
                            {{ scope.row.totalPair }}
                        </template>
                    </el-table-column>
                    <el-table-column align="left" label="成功配对数" min-width="10%">
                        <template slot-scope="scope">
                            {{ scope.row.successPair }}
                        </template>
                    </el-table-column>
                    <el-table-column align="left" label="配对成功率" min-width="10%">
                        <template slot-scope="scope">
                            {{ scope.row.pairingSuccessRate * 100 + '%' }}
                        </template>
                    </el-table-column>
                </el-table>
            </el-main>
...
import { getBlePairData } from '@/api/api-stat'
...
export default {
    ...
    methods: {
        getData(param) {
            startLoading()
            getBlePairData(param).then(response => {
