<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">魔镜</div>
...
import {generateMagicMirror, getMagicMirror,getMagicMirrorImage} from '@/api/agents'
...
export default {
  ...
  methods: {
    ...
    getList(currentPage) {
      getMagicMirror(4).then(response => {
    ...
    getUpload() {
      // get 上传地址
      getMagicMirrorImage().then(response => {
    ...
    async uploadToS3(request) {
      ...
      try {
        ...
        const response = await getMagicMirrorImage()
    ...
    queryAction() {
      ...
      generateMagicMirror(param).then(response => {
        if (response.code === 0) {
          // 获取 requestId
    ...
    checkAndDownloadFile: function(requestId, manual = true) {
      ...
      getMagicMirror(requestId).then(response => {
