<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">猫狗印记</div>
    ...
    <div style="margin: 30px">
      ...
      <el-descriptions :column="1" title="结果">
        <el-descriptions-item label="描述"> {{ this.result.label }}</el-descriptions-item>
...
import { generatePetImprint, getPetImprint } from '@/api/agents'
...
export default {
  ...
  data() {
    return {
      uploadBaseUrl: baseUrl + '/api/agents/pet_imprint/image',
      form: {
        name: '',
        type: 1,
        env: 'test'
  ...
  methods: {
    ...
    queryAction() {
      ...
      generatePetImprint(param).then(response => {
        if (response.code === 0) {
          // 获取 requestId
    ...
    checkAndDownloadFile: function(requestId, manual = true) {
      ...
      getPetImprint(requestId).then(response => {
        ...
        if (data.status !== 0) {
          ...
          this.result.label = this.result.result.description
          return
        }
        if (timerElement) {
