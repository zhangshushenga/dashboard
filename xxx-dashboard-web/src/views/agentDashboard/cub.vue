...
export default {
    ...
    methods: {
        ...
        initAmbientLight() {
            //初始化环境光
            this.light = new THREE.AmbientLight(0xffffff, 0.2, 100);
        ...
        initCamera() {
            ...
            // 正交相机参数
            const frustumSize = 10; // 视锥体尺寸
            const aspect = (window.innerWidth) / (window.innerHeight * 0.9); // 窗口宽高比
            ...
            const distance = 4.5;
            this.camera.position.set(distance, 3, distance); // 设置相机位置
        ...
        initCenterCube() {
            ...
            const geometry = new THREE.BoxGeometry(centerCubeSize, 5, centerCubeSize)
            ...
            const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, (window.innerHeight * 0.9)), 1.5, 0.4, 0.85);
        ...
        initSmallCubes() {
            ...
            for (let i = 0; i < totalCalls + agentTotalNum; i++) {
                ...
                if (i < agentTotalNum) {
                    ...
                    // 另一部分表示当前累计调用量
                    smallCubeMaterial = new THREE.MeshStandardMaterial({ color: 0x54545B });
        ...
        animate() {
            ...
            this.cube.position.y = Math.sin(Date.now() * 0.003) * maxAmplitude * preAgentsCall

            //更新小立方体的位置，使其围绕大立方体旋转
            const rotationSpeed = this.smallCubRoatSpeed; // 旋转速度，调整此值以改变旋转速率
        ...
        onWindowResize() {
            ...
            this.renderer.setSize(window.innerWidth, window.innerHeight * 0.9)
            this.composer.setSize(window.innerWidth, window.innerHeight * 0.9);
        }
    }
}
</script>
