<template>
    <div class="container">
        <div class="text" style="font-size: 35px">验证测试Text结果列表</div>
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" class="table" :fit="true" :show-overflow="'tooltip'"
            ...
            <el-table-column prop="task_type" label="任务类型" min-width="10%">
            </el-table-column>
            
            <el-table-column prop="textEvaledCount" label="验证完成数" min-width="10%">
            </el-table-column>
            <el-table-column prop="textTotalCount" label="文件内总数量" min-width="10%">
            </el-table-column>
            <el-progress :text-inside="true" :stroke-width="26" :percentage="70"></el-progress>
            <el-table-column prop="score" label="得分" min-width="10%">
            </el-table-column>
...
import { getTextEvalResultList, getTextTestResultDownloadUrl } from '../../api/api-aiep'
...
export default {
    ...
    data() {
        return {
            ...
            editDataSetId: '',
            value:'',
            showTaskType: [{"id": 1, "name": "意图识别"}, {"id": 2, "name": "日语翻译"}]
        }
    },
    mounted() {
        this.firstSearch()
    ...
    methods: {
        ...
        searchData(currentPage) {
            ...
            getTextEvalResultList(param).then(response => {
                ...
                if (response.code === 0) {
                    ...
                    for (const r of response.data.records) {
                        ...
                        for (var j=0; j<this.showTaskType.length; j++){
                            if (this.showTaskType[j].id === r.taskType){
                                r.task_type = this.showTaskType[j].name
                            }
                        }
