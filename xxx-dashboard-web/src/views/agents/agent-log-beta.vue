<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">Beta Agent使用记录</div>
    <div style="display: flex; flex-direction: row;">
      <div style="display: flex; flex-direction: row; flex-wrap: wrap; flex: 7">
        ...
        <el-select v-model="agentTypeList" class="queryItem" clearable multiple placeholder="选择模型类型" @change="handleChange">
          <el-option
    ...
    <div style="display: flex; flex-direction: row; flex-wrap: wrap;">
      <div v-for="(data, index) in currentData" :key="data.id" class="imageCard">
        ...
        <div class="content">Version: {{ data.param ? JSON.parse(data.param).version :'' }}</div>
...
import {
  getBetaAgentLogList, getAgentBetaSelectList
...
export default {
  ...
  methods: {
    ...
    getSelectList(currentPage) {
      getAgentBetaSelectList().then(response => {
    ...
    getList(currentPage) {
      ...
      console.log(param)
      getBetaAgentLogList(param).then(response => {
        endLoading()
        console.log(response)
        if (response.code === 0) {
          this.totalData = response.data.total
          this.dataSource[currentPage.toString()] = response.data.records.map((obj) => {
            return {
              ...obj,
              'dynamicRoiList': null,
              'llamaDescription': ''
