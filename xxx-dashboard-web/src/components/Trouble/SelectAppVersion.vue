<template>
  <div @mousemove="handleMouseMove">
    <el-select
      ref="mySelect"
      v-model="childAppVersionList"
      :placeholder="$t('label41')"
      allow-create
      class="tag-select"
      clearable
...
export default {
  name: 'SelectAppVersion',
  model: {
    prop: 'fatherAppVersionList', // 父组件v-model绑定的值传递给props中的fatherValue
    event: 'childValueChange'
  },
  props: {
    fatherAppVersionList: Array

  },
  data() {
    return {
      fwList: [],
      childAppVersionList: this.fatherAppVersionList,
      preX: 0
    }
  },
  watch: {
    fatherAppVersionList(newVal, oldVal) {
      this.childAppVersionList = newVal
  ...
  methods: {
    childValueChange() {
      this.$emit('childValueChange', this.childAppVersionList)
    },
    setFw() {
      // this.fwList.push({
      //   label: '1.0.0',
