<template>
  <div style="width: 100%; min-height: 100dvh;">
    <div style="font-size: 26px; font-weight: 600; margin: 20px">宠物互动</div>
...
export default {
  ...
  methods: {
    ...
    getUpload() {
      // get 上传地址
      getPetMomentImage().then(response => {
    ...
    async uploadToS3(request) {
      ...
      try {
        ...
        const response = await getPetMomentImage()
    ...
    queryAction() {
      ...
      generatePetMoment(param).then(response => {
        if (response.code === 0) {
          // 获取 requestId
    ...
    checkAndDownloadFile: function(requestId, manual = true) {
      ...
      getPetMoment(requestId).then(response => {
